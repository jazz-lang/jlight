use crate::heap::block::BLOCK_SIZE;
use num_cpus;
use std::cmp::max;
use std::env;
...
macro_rules! set_from_env {
    ($config:expr, $field:ident, $key:expr, $value_type:ty) => {{
        if let Ok(raw_value) = env::var(concat!("JLIGHT_", $key)) {
...
impl Config {
    ...
    #[cfg_attr(
        ...
        allow(cyclomatic_complexity, clippy::cognitive_complexity)
    )]
    pub fn populate_from_env(&mut self) {
        ...
        set_from_env!(self, heap_growth_threshold, "HEAP_GROWTH_THRESHOLD", f64);
