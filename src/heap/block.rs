...
impl BlockHeader {
    ...
    pub fn set_next(&mut self, block: Ptr<Block>) {
        self.next = block;
    }
...
impl Block {
    ...
    pub fn boxed() -> Box<Block> {
        ...
        let mut block = Box::new(Block {
            ...
            free_pointer: Ptr::null(),
            end_pointer: Ptr::null(),
        });

        block.free_pointer = Ptr::from_pointer(block.start_address());
        block.end_pointer = Ptr::from_pointer(block.end_address());
    ...
    fn pointer_for_hole_starting_at_line(&self, line: usize) -> RawObjectPointer {
