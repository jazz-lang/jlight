...
pub fn simplify_cfg(code: &mut Vec<BasicBlock>) {
    ...
    {
        let unused_blocks: BTreeSet<usize> = (0..code.len()).filter(|i| !dfs_visited[*i]).collect();
...
pub fn remove_bad_moves(code: &mut Vec<BasicBlock>) {
    for bb in code.iter_mut() {
        bb.instructions.retain(|x| match x {
